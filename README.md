...
> **`SERVER_PRIVATE_KEY` / `SERVER_ADDRESS` is the wallet that deployed `GamePayment`** (i.e. its `owner`) — only that key may call `buyItemX402`. There is no buyer key to configure on the server; the buyer side lives entirely in the Unity client.
...
> **`SERVER_PRIVATE_KEY` / `SERVER_ADDRESS`就是部署 `GamePayment` 合约的钱包**(即其 `owner`)——只有该私钥能调用 `buyItemX402`。server 端不需要配置任何买家私钥,买家逻辑完全在 Unity 客户端内。
...
> **`SERVER_PRIVATE_KEY` / `SERVER_ADDRESS`就是部署 `GamePayment` 合約的錢包**(即其 `owner`)——只有該私鑰能呼叫 `buyItemX402`。server 端不需要設定任何買家私鑰,買家邏輯完全在 Unity 客戶端內。
